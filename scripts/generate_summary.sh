...
.expanded-row-content {white-space:pre; font-size: 12px !important; color: #777; border-top: none; display: grid; grid-column: 1/-1; font-size: 13px;} .hide-row { display: none;} #issuetable tr{cursor: pointer; display: grid; border-bottom: 1px solid #e1e1e1; grid-template-columns: 50px 300px 300px 50px 150px 50px;} #issuetable th,td {border:none;}
    </style>
    <script>
...
then
    ...
    while IFS= read -r line ; do 
      ...
      echo $line | sed 's~_#~</td><td>~g' | sed 's/\s+/,/' |sed 's/#@null@#/-/g' | sed 's/null/-/g' |sed 's/#@/\<a href="/g' |sed 's/@#/" target="_blank">Go\<\/a>/g'|sed 's~_^~</td><td class="expanded-row-content hide-row">~g'  >> $file;
    done <<< "$unique_issues"
    printf "</table>" >>  $file;
fi
